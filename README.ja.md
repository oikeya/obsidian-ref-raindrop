...
- Obsidian設定画面から token / folder / AI provider / ignore file を設定
- sync実行時に ignore file を毎回読み直す
...
- `Ignore file path`: 既定 `~/.config/ref-raindrop/.ai_summarize_ignore`
...
AI要約時に絶対アクセスしたくない host は次に書きます。
...
このリポジトリにはテンプレートとして `.ai_summarize_ignore.example` を置いています。実際のignoreファイルはGitに入れないでください。
