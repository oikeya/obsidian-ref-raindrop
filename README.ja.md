...
RefRaindrop は MIT License に基づき、現状有姿で提供されます。Raindrop.io、OpenAI、Gemini、Ollama、ブックマーク先Webサイトへのアクセスなど、連携する外部サービスの設定・利用状況・課金・制限の確認は利用者の責任で行ってください。

AI provider や SaaS API は課金が発生したり、quota / rate limit エラーになったり、仕様や挙動が変わることがあります。RefRaindrop は throttling、ignored hosts、private network block、retry 処理を備えていますが、費用上限、可用性、AI出力の正確性、すべてのブックマーク先へのアクセス安全性を保証するものではありません。大量取り込みやAI indexを実行する前に、設定内容を確認してください。
...
Raindrop同期はAPI上限の50件/pageで取得します。初回フル同期後は、Raindrop側の `lastUpdate` がVault内に保存済みの最新 `raindrop_last_update` に到達した時点で取得を止めます。保存先フォルダを削除した場合や空の場合は、次回同期で全件取得します。
